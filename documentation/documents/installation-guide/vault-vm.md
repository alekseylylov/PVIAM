...
| rds-server-keystore.p12.base64 | База сертификатов/ключей keycloak в которой содержится сертификат для организации https на rds-server                                                          |
| ttl                            | задается частота обновления с сервера секретов на Vault Agent (например 1h)                                                                                    |
...
> (в последнем случае vault_approle_id и vault_approle_secret_id не нужно задавать в профиле установки,
