...
Включение опциональных конфигурационных файлов ("include common/*.conf") задается через RDS Server (при его наличии) в
переменной `applyJctRequestFilter` (несколько файлов-опций указывается через запятую) или через профиль развертывания, в
