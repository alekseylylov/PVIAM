# Вход под рядовым пользователем на ответвлении для привилегированного пользователя
...
2. **Действие**:
   ...
   Выполнить вход на защищенный ресурс под рядовым пользователем (на стенде настроено ответвление для входа по 2ФА, вход под альтернативным client_id).

   **Успешный результат**:

   Получена ошибка при аутентификации:
   Текущая аутентификация/id-token не подходит для доступа к этому ресурсу.Попробуйте [выполнить реаутентификацию](https://platform-da-dp01-sc-auth-ift-2.apps.dap.devpub-01.solution.sbt/jct-snoop-alt/snoop/?needReauthentification=true).
   Ошибка: атрибут 'azp' в id-token не равен client_id, с которым необходима аутентификация (id_token.azp=PlatformAuth-Proxy, client_id=PlatformAuth-Proxy-Alt)
   Повторить [вход в систему](https://platform-da-dp01-sc-auth-ift-2.apps.dap.devpub-01.solution.sbt/).