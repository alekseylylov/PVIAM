...
> Примечание
>
> На VM вместо `/iamproxy/...` необходимо использовать пути `/opt/iamproxy/...`.
...
- `PROXY_REQUEST_TO_API_REGEX` - опциональный, default `/api/`, регулярное выражение применяемое к URI запроса, при
  ...
  кодом 401;
- `PROXY_HEALTH_URI_PATH` - опциональный, default `/health`, путь до ендпоинта вывода статуса здоровья IAM Proxy на
  фронте(в формате json);
...
- `DEBUG` - не пустое значение включает расширенное логирование (как в SynGX, так и на уровне bash-скриптов), значение
  ...
  контейнере.
- `CONTAINER_EXEC_MODE` - по умолчанию `''`(пустая строка), возможные значения `''`/`only_proxy`/`only_conf`, включает
  режим запуска только части функциональности контейнера IAM Proxy (позволяет разделить формирование конфигурации и работу
  прокси на разные контейнеры). Значение `''` - запуск в обычном режиме (все в одном контейнере), `only_proxy` - только
  запуск прокси на готовой конфигурации, `only_conf` - только формирование конфигурации и ее последующее обновление в
  runtime (в том числе поддержка Hot Reload).

### Параметры подключения к провайдеру аутентификации платформы (Platform V IAM KeyCloak.SE)
...
IDP-провайдера аутентификации, так и в сторону бэкенд-приложения. Механизм защиты основан на конфигурируемом ограничении
скорости по количеству запросов в единицу времени. В случае превышения количества запросов они будут задерживаться для
соблюдения заданного лимита, или пользователю будет показано сообщение, о необходимости повторения попытки через
некоторое время.
...
- `PROXY_LIMIT_REQ_PER_SEC` - запросов/секунду, опциональный, default `60`, ограничение скорости запросов на бэкенд в
  количестве запросов допустимых в секунду (допускается всплеск запросов по количеству в 4 раза превышающий заданный лимит
...
- `PROXY_LIMIT_REQ_OIDC_PER_MIN` - запросов/минуту, опциональный, default `2`, ограничение скорости запросов по сессии
  ...
  запросов в минуту (допускается всплеск в количестве 10 запросов, по превышению всплеска запросы будут отклоняться с HTTP
  кодом 509).
...
- `OIDC_USERINFO_MAP` - задать список атрибутов из `userinfo` (полученных из URI userinfo API IDP) для сохранения(
  кеширования) их в контексте сессии IAM Proxy, например
...
- `md5` - вычисление хеш-суммы md5 от текста или данных;
...
отправлять периодически запрос по HTTP URL до метаданных основного провайдера, чтобы проверить его
состояние/доступность. В случае недоступности основного провайдера, аутентификация начинает временно выполняться на
альтернативном провайдере, до восстановления работоспособности основного.
...
разместить по пути `/iamproxy/conf/blacklist/blacklist-useragent.txt` (используя монтирование ConfigMap в файл), и
