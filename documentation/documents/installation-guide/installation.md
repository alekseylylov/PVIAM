...
[Шаг 1 Подготовка окружения (обязательный)](#1)

[Шаг 2 Подготовка дистрибутива (обязательный)](#2)

[Шаг 3 Создание/изменение профиля развертывания (обязательный)](#3)

[Шаг 4 Настройка конфигурационных параметров (обязательный)](#4)

[Шаг 5 Выполнение развертывания (обязательный)](#5)

[Шаг 6 Проверка результатов (обязательный)](#6)
...
| `rds_server_geo1`, `rds_server_geo2` | указываются сервера RDS Server (может не содержать серверов)                                                                                                                  |
| `rds_server`                         | группа включающая в себя все группы или сервера RDS Server                                                                                                                    |
...
| `proxy_use_configuration_from_rds`         | Получать конфигурацию ответвлений из API `RDS Server` (будет выполнена установка службы rds-client рядом с прокси)                                                                                                                                                                                                     | proxy_use_configuration_from_rds: True                                                                               |
...
| `proxy_jct_list`                           | Описание параметров ответвлений (junctions), описание приведено ниже в разделе "Заполнение раздела с описанием параметров ответвлений (junctions)"                                                                                                                                                                     | Пример смотрите ниже                                                                                                 |
| `rds_server_urls`                          | Указывается список URL rds-серверов через «;» по которым отдается активная конфигурация, используется клиентская failover-балансировка                                                                                                                                                                                 | rds_server_urls: https://rds-server.mycompany.ru/rds-for-proxy/active-conf-json                                      |
...
##### Заполнение раздела с описанием параметров ответвлений (junctions)
...
###### Файлы дополнительных опций для ответвлений (junctions)

Файлы-опции указанные ниже можно указывать в параметре ответвлений `applyJctRequestFilter` (можно указать в профиле
развертывания, или в компоненте PACMAN (CFGA), или в yaml-файле для RDS Server, несколько опций указываются через
запятую, пробелы по краям игнорируются):
...
| common/rds-cache-control-no-store.location.conf              | Отключение кэширования на агенте/браузере (приводит к заданию в ответе заголовков `Cache-Control`, `Expires`, `Pragma`)                                                                                                                                                                                                                                                                                                  |
| common/rds-cache-control-off.location.conf                   | Отключение изменения заголовков отвечающих за кэширования на агенте/браузере (заголовков `Cache-Control`, `Expires`, `Pragma`)                                                                                                                                                                                                                                                                                           |
...
| common/rds-keep-cookie-path.location.conf                    | Не изменять path в заголовке Set-Cookie. Если имена cookie начинаются с `__Host-`, эта опция помогает сохранить такие куки на непрозрачных ответвлениях.                                                                                                                                                                                                                                                                 |
...
- `resolve` - отслеживает на DNS сервере изменения IP-адресов, соответствующих доменному имени сервера, и автоматически 
  ...
  будет ошибка разрешения fqdn имени). Данная опция включена по умолчанию, выключить её можно через опцию 
...
- keycloak:
  ...
  - `smtpServer` - задать параметры подключения к почтовому серверу по SMTP и пользователя, которые будут использоваться
    Keycloak при отсылке уведомлений по email;
  - `realms` - опциональный, задает список realms для пользователей, создаваемых развертыванием. По умолчанию создается
    один пользовательский realm - `PlatformAuth`;
  > Пример:
  > ```yaml
  ...
  - `userRealmOptions.afterCreateUserSendEmail` - принимает значения `True`/`False`.
    * значение `True` - при создании пользователя отправляет уведомление по email и одноразовый временный токен на смену
      пароля.
    * значение `False` - при создании пользователя не отправляет уведомление по email и одноразовый временный токен на
      смену пароля;
  ...
  - `soapApi.verifyCN` - SOAP API по управлению УЗ требует аутентификации по клиентскому сертификату, в данном параметре
    указывается список допустимых CN клиентского сертификата (при mTLS) через `|`, `*` - отключить проверку;
  - `soapApi.rolesFilter` - фильтр по scope ролей, подпадающих под синхронизацию через API. Можно указать несколько
    ...
    - `realm_options` - опциональный, список пар значений `name`+`value`, которые будут установлены как опции для realm
      (список доступных опций необходимо смотреть в документации компонента KCSE); Пример:
      > ```yaml
      > realm_options:
...
- `proxy_mtls_cert_file` - файл сертификата прокси, для организации mTLS между прокси и проксируемой
  подсистемой/бэкенд. Не обязательный;
...
- `proxy_healthcheck_enable` - опциональный. `True`/`False`, по умолчанию `False`. `True` - включение активного
  `healthcheck` до серверов бэкенд (так же надо включить healthcheck на конкретном ответвлении, параметры смотрите
  выше, в разделе "Заполнение раздела с описанием параметров ответвлений (junctions)");
...
- `proxy_support_isam_headers` - опциональный, `default True`, `True` - добавлять в запросы HTTP-заголовки аналогично
  `ISAM/WebSeal` (`iv-user`, `iv-groups`, `iv-remote-address`). Функциональность является устаревшей (deprecated),
  и поддержка данной функциональности будет прекращена в релизе Platform V IAM SE 5.0.0;
...
> - `authz_spas_rights_lifetime` - опциональный, частота обновления полномочий из функциональной подсистемы Авторизации, в
    секундах;
...
#### Заполнение файла `ansible/inventories/StandX/group_vars/rds_server.yml`

Использование RDS Server требуется, только если есть необходимость динамического конфигурирования ответвлений, 
и тогда прокси будет получать настройки ответвлений через RDS API (с использованием RDS Client) и применять их в 
действие в течение нескольких секунд после изменений.
Без использования RDS API изменения конфигурации потребуют перезапуска установки (выполнения ansible-playbook).

Динамическое конфигурирование ответвлений может быть полезно при большом количестве ответвлений(более 10), и/или при 
необходимости частого изменения параметров ответвлений.

Для настройки RDS Server необходимо задать параметры в YAML для путей указанных ниже:
...
- `rds.standin.*`
- `rds.server.*`
- `rds.configStore.config-store-jdbc-login` - логин к БД компонента PACMAN (CFGA)
  продукта Platform V Configuration (CFG).
- `rds.configStore.config-store-jdbc-url` - url для подключения к БД Platform V Configuration компонента PACMAN
  (пример `jdbc:postgresql://10.x.x.3:5432/config`).
- `rds.configStore.configStoreCryptoPas` - пароль для шифрования в Platform V Configuration компонента PACMAN (параметр,
  влияющий на безопасность).
- `rds.audit.rest.enabled` - сервера для подключения к kafka Platform V Audit SE (пример
  `10.x.x.48:9092,10.x.x.179:9092`).
- `rds.audit.mockMode` - true/false, включение режима заглушки Platform V Audit SE.
- `rds.audit.runWithoutAudit` - `true`/`false`, `true` - продолжить работу приложения при проблемах подключения к
  Platform V Audit SE.
...
- `rds.server.ssl.client-auth` - `none`/`need`, использовать аутентификацию по клиентскому сертификату при обращении к
  API;
- `swidtag_location_rds` - параметр отвечает за конфигурацию пути до файла, содержащего swid-tag на VM.

> Примечания
>
> Все параметры из узла `rds` (это параметры `rds.*`) будут при установке переданы в корень
> конфигурации приложения RDS Server (конфигурации springboot). Например, параметр `rds.audit.rest.enabled` будет
> передан в конфигурацию приложения по пути `audit.rest.enabled`.
>
> Список параметров развертывания указанный выше **не исчерпывающий**, указаны параметры на которые стоит
> обратить внимание.
>
> Подробней про настройки RDS Server описано в разделе
> [Описание настроек приложения RDS Server](../additional-materials/rds-server-configuration.md).
>
> Полный перечень параметров используемых для установки в разрезе сред и инструментов установки,
> приведен в разделе [Соответствие имен параметров для разных сред и инструментов установки](params.md).
...
- параметры задать в `ansible/inventories/StandX/group_vars/all/vault_main_decrypted.yml` (игнорируется git, и хранится
  ...
  - `vault_rds_config_store_jdbc_pas` - пароль к БД компонента PACMAN.
- зашифровать файл с помощью ansible-vault, для этого:
  ...
  - записать vault-пароль в `ansible/inventories/StandX/group_vars/all/vault-password.txt` (игнорируется git, и хранится
    только локально);
...
- следующие файлы размещаются в каталоге `ansible/inventories/StandX/files/` (необходимость конкретного файла зависит от
  ...
  - `proxy-server.p12` - из этой базы rds-client берет клиентский сертификат для mTLS при подключении по HTTPs к
    rds-server (обычно там тот же сертификат, что и в `proxy-server.*.pem`, но в формате PKCS12);
  ...
  - `rds-server-keystore.p12` - база сертификатов/ключей keycloak, в которой содержится сертификат для организации HTTPs
    на rds-server;
...
- `rds-server/*` - файлы и подкаталоги из этого каталога будут доставлены до серверов rds_server.
...
3. Перейти в Jenkins и создать Jenkins job c типом `Pipeline\<img height="300px" src="resources/new_item.png"
   /><img height="300px" src="resources/pipeline.png"/>`.
...
9. После обновления страницы появится кнопка `Собрать с параметрами`, по которой можно будет запустить сборку с
   ...
   - `SSH_USER`- опциональный параметр, учетная запись ssh-пользователя для подключения к хостам, ее необходимо указывать
     __только__ при запуске подготовки серверов, когда необходимы привилегии `sudo root`;
   - `LIMIT`- опциональный параметр, задать ansible параметр `limit` для ограничения установки на выбранные хосты/группы,
     которые расположены файле hosts (например, когда нужно установить новую версию на часть серверов);
...
- `prep-deployers.sh` - выполняется на всех серверах в первую очередь. При запуске скрипта необходимо будет задать
  пароль для ssh-пользователя (`iamproxy-deployer`), под которым будет производиться развертывание (значение из
  переменной профиля `vault_ansible_ssh_pass`). Так же пароль можно указать при запуске `prep-deployers.sh` в первом
  параметре или в переменной окружения `deployer_pass` (параметр, влияющий на безопасность), если пароль не указан при
  запуске он будет запрошен в терминале (журналирования и/или аудита);
...
- `prep-rds-client.sh` - выполняется на всех серверах группы `proxy`, при использовании конфигурирования через
  rds-server и компонент PACMAN (CFGA) продукта Platform V Configuration (CFG);
- `prep-rds-server.sh` - выполняется на всех серверах группы `rds_server`, при использовании конфигурирования через
  rds-server и компонент PACMAN (CFGA) продукта Platform V Configuration (CFG);
- `prep-fluent-bit.sh` - выполняется на всех серверах группы proxy, rds_server, keycloak при условии, что существует
  переменная `logger_options`;
...
Описание прав linux-пользователей, которые создаются скриптами подготовки - `prep-rds-server.sh`
, `prep-keycloak.sh`, `prep-iamproxy.sh`, `prep-deployers-dev.sh`,`prep-deployers.sh`,`prep-fluent-bit.sh`
, `prep-vault-agent.sh`.

| Учетная запись | Описание полномочий и функциональности                                                                     | Объекты доступа                                           |
|----------------|------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|
| deployer       | Права запуска команд под пользователем службы                                                              | users: rds-server, keycloak, iamproxy, fluent-bit, vault  |
...
| rds-server     | Чтение и запись объектов, управление службой rds-server                                                    | /opt/rds-server                                           |
