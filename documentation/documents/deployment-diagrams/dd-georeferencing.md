# Схема развертывания с георезервированием
...
3. RDS Server является адаптером получения конфигурации для Proxy из компонента PACMAN (CFGA). RDS не решает
   задачу реконселяции сессионных данных между двумя ЦОДами. Исходя из архитектуры, это задача компонента PACMAN
   (CFGA).
4. Во всех ЦОДах используется одинаковая прикладная конфигурация настройки ответвлений IAM Proxy.
...
5. Выбор активного RDS Server определяется конфигурациями развертывания, которые определяют перечень RDS-серверов,
   используемых в RDS Client.
6. Инфраструктурные балансировщики предоставляются инфраструктурой на стороне заказчика (настройка, сопровождение и
