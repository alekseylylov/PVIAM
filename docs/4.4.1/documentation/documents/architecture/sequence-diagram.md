...
[Описание API для реализации динамической настройки IAM Proxy](../additional-materials/rds-api.md), и в разделе
...
alt Если в запросе есть заголовок Authorization
  alt#Gold Получение токенов из локального кэша
    Proxy -> Proxy: Поиск токенов в локальном кэше по ключу из Authorization
  else #LightGreen Если не найдено токенов в кэше, то получение токенов от IDP
    ...
    Proxy -> Proxy: Сохранение токенов в локальном кэше на срок жизни access токена
  end
  break #Pink Если не получили токены
...
Про описание и настройку этого функционала смотрите в
...
Диаграмма описывает взаимодействия при использовании функционала переключения на альтернативный провайдер, возникающие в
сценариях:
...
alt Если в запросе есть заголовок Authorization (режим direct-auth)
  alt Получение токенов из локального кэша
    Proxy -> Proxy: Поиск токенов в локальном кэше \n по hash от Authorization
  else Если не найдено токенов в кэше, то получение токенов от IDP
    Proxy -> IDP: Запрос на получение токенов + заголовок Authorization
    activate IDP
    IDP -> IDP: Аутентификация пользователя по данным \n из заголовка Authorization \n (например по Basic Authentication)
    Proxy <-- IDP: Передача токенов: access, refresh, ID
    deactivate IDP
    Proxy -> Proxy: Сохранение токенов в локальном кэше \n на срок жизни access-токена
