...
IAMTokenAuthenticatorProvider, в среде контейнеризации настраивается объект с типом ConfigMap, содержащий открытый ключ
IAM, на котором производится проверка подписи токена от IAM.

На рисунке ниже приведен пример уже смонтированного ConfigMap в папку контейнера (в файл).

![](resources/image2021-2-10_23-42-50.png)

Открытый ключ IAM доставляется до ConfigMap job Jenkins при развертывании приложения.
